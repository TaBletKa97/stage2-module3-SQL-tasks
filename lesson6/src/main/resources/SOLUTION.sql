...
select * from MARK where STUDENT_ID = (select id from SUBJECT where NAME = 'Art');
...
select distinct s.* from STUDENT as s join MARK M2 on s.ID = M2.STUDENT_ID where MARK is not null;